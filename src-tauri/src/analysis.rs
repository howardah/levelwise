...
pub(crate) fn measure_file(mut progress: impl FnMut(u8), path: &Path) -> Result<Analysis, String> {
    let file = File::open(path).map_err(|error| format!("Unable to open file: {error}"))?;
    ...
    let probed = symphonia::default::get_probe()
        ...
        .map_err(|error| format!("Unsupported audio file: {error}"))?;
    ...
    .map_err(|error| format!("Could not initialize loudness meter: {error}"))?;
    let mut duration_frames: u64 = 0;
    let mut last_progress = 0;
    // Most compressed formats decode to f32. Other formats reuse this conversion buffer.
    ...
    loop {
        let packet = match format.next_packet() {
            ...
            Err(error) => return Err(format!("Error reading audio data: {error}")),
        };
        ...
        let decoded = match decoder.decode(&packet) {
            ...
            Err(error) => return Err(format!("Error decoding audio: {error}")),
        };
    ...
    let integrated_lufs = meter
        .loudness_global()
        .map_err(|error| format!("Unable to calculate integrated loudness: {error}"))?;
...
#[cfg(test)]
mod tests {
    use super::{db, measure_file};
    use std::path::Path;

    #[test]
    fn decibels_have_a_safe_floor() {
        assert_eq!(db(0.0), -120.0);
        assert_eq!(db(-1.0), -120.0);
    }

    #[test]
    fn converts_linear_amplitude_to_decibels() {
        assert!((db(1.0) - 0.0).abs() < f64::EPSILON);
        assert!((db(0.5) - -6.020_599_913).abs() < 1e-9);
    }

    #[test]
    fn missing_files_return_a_useful_error() {
        let error = measure_file(|_| {}, Path::new("definitely-not-an-audio-file.wav"))
            .expect_err("a missing file should fail");
        assert!(error.starts_with("Unable to open file:"));
    }
}
