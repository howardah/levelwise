...
const adjustment = (target: number, canBoost: boolean, loudness = integrated.value) =>
  gainAdjustment(target, canBoost, loudness);
...
async function analyzeFile(path: string) {
  ...
  progress.value = 0;
  const unlisten = await listen<number>("analysis-progress", ({ payload }) => {
...
async function togglePreview() {
  ...
  setupPreviewGain();
  if (player?.dataset.path !== selectedFile.value.path)
    await switchPreview(selectedFile.value, false);
  audioContext?.resume();
  currentPlayer()
    ?.play()
    .catch(() => {
      error.value = "Playback could not start for this file.";
    });
}
...
async function switchPreview(recording: PreviewTrack, crossfade: boolean) {
  ...
  if (!crossfade || !outgoing || !audioContext || !gainNodes.length) {
    if (gainNodes[nextDeck] && audioContext)
      gainNodes[nextDeck].gain.setValueAtTime(
        Math.pow(10, selectedPenalty.value / 20),
  ...
  gainNodes[activeDeck === 0 ? 1 : 0].gain.setValueAtTime(
    gainNodes[activeDeck === 0 ? 1 : 0].gain.value,
    now,
  );
...
function seekPreview(event: MouseEvent) {
  ...
  const position =
    Math.max(0, Math.min(1, (event.clientX - bounds.left) / bounds.width)) * duration.value;
...
async function buildWaveform(recording: PreviewTrack) {
  ...
  try {
    ...
    const bars = 88,
      block = Math.max(1, Math.floor(decoded.length / bars));
    ...
    if (selectedFile.value?.path === recording.path)
      waveform.value = values.map((value) =>
        Math.max(8, Math.round(100 * Math.sqrt(value / maximum))),
      );
    if (!audioContext) await context.close();
  } catch {
    if (selectedFile.value?.path === recording.path) waveform.value = [];
  } finally {
    if (selectedFile.value?.path === recording.path) waveformLoading.value = false;
  }
}
function reset() {
...
<template>
  <main>
    ...
    <section class="hero">
      ...
      <p class="lede">
        Analyze one master or compare a batch — then preview how streaming services<br />will
        normalize each recording without uploading a thing.
      </p>
    </section>
    ...
    <section v-if="!selectedFile" class="upload-shell">
      ...
      <div class="supported">
        <span>LOCAL · PRIVATE · NO ACCOUNT</span>
        <div></div>
        <span>BUILT FOR MASTERING</span>
      </div>
    </section>
    ...
    <section v-else class="results-wrap">
      <div class="filebar">
        ...
        <div>
          ...
          ><span>{{ durationText }} · analyzed in Rust</span>
        </div>
      ...
      <template v-if="!processing || recordings.length">
        <div class="metrics">
          <article>
            <p>INTEGRATED LOUDNESS</p>
            <strong>{{ formatDb(integrated, " LUFS") }}</strong
            ><span>Gated EBU R128 measurement</span>
          </article>
          <article>
            <p>TRUE PEAK</p>
            <strong>{{ formatDb(peak, " dBTP") }}</strong
            ><span>4× oversampled peak scan</span>
          </article>
          <article>
            <p>LOUDNESS RANGE</p>
            <strong>{{ lra?.toFixed(1) ?? "—" }} <b>LU</b></strong
            ><span>EBU Tech 3342 range</span>
          </article>
        </div>
        <div v-if="recordings.length > 1" class="comparison">
          ...
          <button
            ...
            :key="recording.path"
            class="recording-row"
            :class="{ active: selectedFile?.path === recording.path }"
        ...
        <div class="service-grid">
          ...
          >
            ...
            <b
              :class="{
                neutral: adjustment(item.target, item.canBoost) === 0,
                boost: adjustment(item.target, item.canBoost) > 0,
              }"
              >{{ formatDb(adjustment(item.target, item.canBoost), " dB") }}</b
            >
          </button>
        </div>
        <div class="preview">
          ...
          <div class="preview-transport">
            ...
            >
              <i
                ...
                :key="index"
                :class="{ played: index / waveBars.length <= previewTime / Math.max(duration, 1) }"
                :style="{ height: height + '%' }"
              ></i>
              <span
                class="playhead"
                :style="{ left: `${Math.min(100, (previewTime / Math.max(duration, 1)) * 100)}%` }"
              ></span>
            </button>
          ...
          <button
            class="play"
            :aria-label="isPreviewing ? 'Pause preview' : 'Play normalized preview'"
            @click="togglePreview"
          >
            {{ isPreviewing ? "Ⅱ" : "▶" }}
          </button>
          <audio
            ref="playerA"
            @play="activeDeck === 0 && (isPreviewing = true)"
            @pause="activeDeck === 0 && (isPreviewing = false)"
            @timeupdate="onTimeUpdate(0)"
            @ended="onPreviewEnd(0)"
          ></audio>
          <audio
            ref="playerB"
            @play="activeDeck === 1 && (isPreviewing = true)"
            @pause="activeDeck === 1 && (isPreviewing = false)"
            @timeupdate="onTimeUpdate(1)"
            @ended="onPreviewEnd(1)"
          ></audio>
        </div>
        <p class="disclaimer">
          EBU R128 / ITU-R BS.1770 analysis is performed locally in the Rust backend. No audio
          leaves your device.
        </p>
      </template>
    </section>
